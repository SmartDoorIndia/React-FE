...
// URL-safe Base64 decode (to retrieve the original integer propertyId)
export const urlSafeBase64Decode = (encodedText) => {
  // Replace URL-safe characters back to standard Base64
  let trimText = encodedText.slice(1, -2); 
  let base64 = trimText.replace(/-/g, '+').replace(/_/g, '/');
  while (base64.length % 4 !== 0) {
    base64 += '='; // Add padding if necessary
  }
  const decodedString = atob(base64); // Decode from Base64
  return parseInt(decodedString, 10); // Convert the decoded string back to an integer
};

const encodePropertyId = (propertyId) => {
  const base64 = btoa(propertyId?.toString());
  // Make Base64 URL-safe
  const urlSafe = base64.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
  return urlSafe;
}

const getRandomAlphabet = (count = 1) => {
...
export const urlSafeBase64Encode = async (pPropertyId) => {
  const encoded = encodePropertyId(pPropertyId);
  const randomPrefix = getRandomAlphabet(1);
  const randomSuffix = getRandomAlphabet(2);
  const obscured = `${randomPrefix}${encoded}${randomSuffix}`;
  return obscured;
}

export const projectShare = async (projectId) => {
   const url = await urlSafeBase64Encode(projectId);
   console.log(url);
   let shareURL = "https://www.smartdoor.app/builder-project-share/" + url;
   return shareURL;
};
...
export function formatPrice(price) {
   if (price >= 10000000) {
      ...
      return price; // Return as is for values below 1 lac
   }
}
