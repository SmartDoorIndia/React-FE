...
class NewTeamMemberPage extends Component {
   ...
   handleValidate = async () => {
      ...
      for (let i = 0; i < this.state.city.length; i++) {
         ...
         const cityObject = allCitiesWithId?.data?.find(
            (city) => Number(city.cityId) === Number(cityName)
         );
      ...
      addNewTeamMember(data)
         .then((response) => {
            if (response.data && response.data.status === 200) {
               ...
               setworkCityRequest({ userId: response?.data?.resourceData, cityIdList: city }).then(
                  (response) => {
                        ...
                        <Col lg="4">
                           <Form.Group controlId="formBasicContact">
                              ...
                              <Form.Control
                                 ...
                                 onChange={(e) =>
                                    this.setState({
                                       executiveName: e.target.value.replace(/[^A-Za-z\s]/g, ""),
                                    })
                                 }
                              />
                           </Form.Group>
                        ...
                        <Col lg="4">
                           <Form.Group>
                              ...
                              <Form.Control
                                 ...
                                 onChange={(e) =>
                                    this.setState({
                                       alternatePhoneNumber: e.target.value.slice(0, 10),
                                    })
                                 }
                              />
                           </Form.Group>
                              ...
                              <Form.Control
                                 ...
                                 onChange={(e) => {
                                    if (post.length && e.target.value.length) {
                                       this.props.getExecutivesWrtRoleLocation({
                                          role: Number(post),
                                          page: 1,
                                          size: "",
                                          city: [e.target.value],
