...
class NewPlan extends Component {
   ...
   constructor(props) {
	  ...
	  this.state = {
		 ...
		 isAutoRenewable:
			...
			"",
		 isMarketingSupport: this?.props?.location?.state?.planData?.marketingSupport,
		 isLeadGeneration: this?.props?.location?.state?.planData?.leadGeneration,
		 isMarketingVideo: this?.props?.location?.state?.planData?.marketingVideo,

		 isDeviceCamera: this?.props?.location?.state?.planData?.deviceCamera,
		 isDeviceDongle: this?.props?.location?.state?.planData?.deviceDongle,
		 isDeviceHub: this?.props?.location?.state?.planData?.deviceHub,
		 isDeviceSensor: this?.props?.location?.state?.planData?.deviceSensor,
		 isDeviceSmartLock: this?.props?.location?.state?.planData?.deviceSmartLock,
		 isAutoDoorCloser: this?.props?.location?.state?.planData?.autoDoorCloser,
   ...
   fileUpload = (event) => {
	  ...
	  if (event?.target?.files?.size > maxSizeInBytes) {
		 showErrorToast("File must be less than 15MB...");
		 return;
	  }
	  formData.append("file", event.target.files[0]);
	  formData.append('enumType', 'PLAN_IMAGES');
	  ...
	  if (event.target.files.length > 0) {
		 uploadImage(formData)
			.then((reponse) => {
			   ...
			   this.setState({ imageLocation: reponse?.data?.resourceData[0] });
					 ...
					 <Row>
						...
						<Col lg="6">
						   <Form.Group>
							  ...
							  <Form.Control type="text" value={imageLocation} disabled />
						   </Form.Group>
						   <Text
							  color="dangerText"
							  size="xSmall"
