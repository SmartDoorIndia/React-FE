...
const ConsumerManagement = (props) => {
   ...
   const [filterText, setFilterText] = React.useState(
      data.length !== 0 ? getAllConsumerUsersData?.data?.searchStr : ""
   );
   ...
   const [currentPage, setCurrentPage] = useState(
      data.length !== 0 ? getAllConsumerUsersData?.data?.currentPage : 1
   );
   const [rowsPerPage, setRowsPerPage] = useState(
      data.length !== 0 ? getAllConsumerUsersData?.data?.rowsPerPage : 8
   );
      ...
      },
      {
         ...
         cell: ({ imageUrl, name, nameMatch }) => (
            <ToolTip position="top" style={{ width: "100%" }} name={name || ""}>
               <Text
                  size="Small"
                  style={{ color: nameMatch ? "#252525" : "#be1452" }}
   ...
   const [defaultSort, setDefaultSort] = useState(
      data.length !== 0 ? getAllConsumerUsersData?.data?.defaultSort : true
   );
   const [defaultSortId, setDefaultSortId] = useState(
      data.length !== 0 ? getAllConsumerUsersData?.data?.defaultSortId : "id"
   );
   const [defaultSortFieldId, setDefaultSortFieldId] = useState(
      data.length !== 0 ? getAllConsumerUsersData?.data?.defaultSortFieldId : 1
   );
   const handleSortedData = (newSortedData) => {
      ...
      dispatch({
         ...
         data: {
            consumersData: [...sorted],
            records: recordSize,
            currentPage: currentPage,
            rowsPerPage: rowsPerPage,
            searchStr: filterText,
            kycStatus: getAllConsumerUsersData?.data?.kycStatus,
            defaultSort: !defaultSort,
            defaultSortId: defaultSortId,
            defaultSortFieldId: defaultSortFieldId,
         },
   ...
   return (
      <>
         ...
         <Modal
            show={consumerInfoModal}
            onHide={() => {
               setConsumerInfoModal(false);
            }}
            centered
               ...
               <div className="d-flex">
                  ...
                  <Col lg="8">
                     <Text
                        ...
                        className="text-start"
                        text={
                           selectedConsumer.karzaName === null ? "-" : selectedConsumer.karzaName
                        }
                     />
                  </Col>
               </div>
