...
const ExecutionListing = (props) => {
    ...
    if (props?.tabName === "Installation/Un-installation Requests") {
      filteredItems = installationReqData.data.length ?
        installationReqData.data.filter(item => {
          return item?.id === Number(filterText)
  ...
  const subHeaderComponentMemo = React.useMemo(() => {
    ...
    return (
      <SearchInput className={'w-100'} placeholder={props?.tabName === "Installation/Un-installation Requests" ? 'Search id,propertyId,Installer name,location' : 'Search here'} onFilter={e => setFilterText(e.target.value)} onClear={handleClear} filterText={filterText} />
    );
