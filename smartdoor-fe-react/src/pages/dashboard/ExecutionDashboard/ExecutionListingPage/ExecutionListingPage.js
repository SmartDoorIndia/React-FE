...
const ExecutionListing = (props) => {
   ...
   const statusArr =
      props?.tabName === "Installation/Un-installation Requests"
         ? CONSTANTS_STATUS.installationRequestsStatusArr
         : [];
   ...
   const [currentPage, setCurrentPage] = useState(
      installationReqData?.data?.length !== 0 ? installationReqData?.data?.currentPage : 1
   );
   const [rowsPerPage, setRowsPerPage] = useState(
      installationReqData?.data?.length !== 0 ? installationReqData?.data?.rowsPerPage : 8
   );
   const recordSize = 1000
   let recordsPerPage = 0;
   recordsPerPage = installationReqData?.data?.rowsPerPage;
   ...
   const InstallationRequestsColumns = [
      ...
      },
      {
         ...
         cell: ({ dateTime, slotTime }) => (
            <span>{`${formateDate(dateTime)} | ${slotTime || "-"}`}</span>
         ),
      },
      {
         name: "Location",
      ...
      },
      {
         ...
         cell: ({ city }) => (
            ...
            <ToolTip position="top" style={{ width: "100%" }} name={city || ""}>
               <span className="cursor-pointer elipsis-text">
                  {" "}
                  {city?.substring(0, city.indexOf(",") !== -1 ? city.indexOf(",") : city.length)}
               </span>
            </ToolTip>
         ),
      },
      {
         ...
         cell: ({ requestFor }) => (
            ...
            <span className="cursor-pointer elipsis-text"> {requestFor}</span>
            // </ToolTip>
         ),
      },
      {
         ...
         cell: ({ corporateName }) => (
            ...
            <ToolTip position="top" style={{ width: "100%" }} name={corporateName || "-"}>
               <span className="cursor-pointer elipsis-text">{corporateName || "-"}</span>
            </ToolTip>
         ),
      },
      {
         ...
         cell: ({ id, assignedTo, userList }) => (
            ...
            <Text
               size="Small"
               color="secondryColor"
               className="text-center elipsis-text"
               text={assignedTo ? assignedTo.capitalizeWord() : "Unassigned"}
            />
         ),
      },
      {
   ...
   const PublishedPropertyColumns = [
      ...
      },
      {
         ...
         cell: ({ propertySubType }) => (
            <Text size="Small" color="secondryColor" text={propertySubType || "-"} />
         ),
      },
      {
         name: "Action",
         center: true,
         cell: ({ smartdoorPropertyId, postedById }) => (
            <div className="action">
               <ToolTip position="left" name="View Details">
                  <span>
                     <Link
                        to={{
                           pathname: "/admin/execution/published-property/property-details",
   ...
   const subHeaderComponentMemo = React.useMemo(() => {
      ...
      return (
         <SearchInput
            ...
            placeholder={
               props?.tabName === "Installation/Un-installation Requests"
                  ? "Search id,propertyId,Installer name,location"
                  : "Search here"
            }
      ...
      <div className="tableBox bg-white">
         ...
         <div className="executionInstallationreqTableWrapper">
            <DataTableComponent
               ...
               defaultSort={
                  props.location.state.module === "Installation Requests" ? defaultSort : false
               }
               defaultSortId={
                  props.location.state.module === "Installation Requests"
                     ? defaultSortId
                     : "smartdoorPropertyId"
               }
            />
...
export default compose(withConnect, memo)(ExecutionListing);
